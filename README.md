...
**Choose Folder** searches all subfolders for `orthofinder-input/`, including BAT runs with `hits/orthofinder-input/` below the selected run folder. Both `.fa` and `.fasta` species files are accepted. Tree and alignment selectors show relative paths to distinguish repeated filenames. Select the run folder (for example, `AT2G31880.1/runs/20260414_1939`) and choose the desired tree. **Choose Files** only includes the files you explicitly select.

When selecting a parent folder containing several runs, **Companion files** defaults to **Selected tree's run/folder**. For BAT paths, the scope is `runs/<run>/` (or the timestamp folder when selected directly), even if the tree is nested further down. Other layouts use the tree's containing folder. The alignment picker, species files, datasets, and experimental JSON follow that scope; changing trees refreshes them and prefers an alignment adjacent to the tree. Missing local files never cause a fallback to another run. To deliberately use files elsewhere, choose **All selected folders (override)**. Flat file selections have no folder metadata, so all explicitly selected files remain available.

Species files resolving to the same species are merged with unique tip IDs. If a tree tip appears under different species, import stops with the conflicting tip, species, and source filenames/paths. This also applies to saved sessions, and a failed import leaves the current workspace and undo history intact. Correct the files or choose the correct scope before loading.
